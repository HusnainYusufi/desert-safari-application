...
export default function SubmitPayment({ onSubmit, onCancel }) {
  const [form, setForm] = React.useState({ amount:'', currency:'AED', method:'BANK', reference:'', periodStart:'', periodEnd:'', notes:'', receipt:null });
  const update = (k,v)=>setForm({...form,[k]:v});
  const onFile = (e)=>{ const file = e.target.files?.[0]; if(!file) return; update('receipt', file.name); };
  ...
  return (
    <div className="px-4 safe-b pt-16">
      <div className="max-w-xl space-y-3">
        ...
        <label className="block">
          <span className="block mb-1 text-sm font-medium text-slate-700">Upload receipt (image/pdf)</span>
