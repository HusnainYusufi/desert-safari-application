    ...
    <div className="px-4 safe-b pt-16">
      <div className="card p-4">
        ...
        <div className="mt-3 flex gap-3">
          ...
          <Button variant="outline" onClick={()=>onExport?.('pdf')}>Statement PDF</Button>
        </div>
      </div>

      <div className="mt-3 space-y-2">
        {filtered.map(p => (
          <button key={p.id} onClick={()=>onOpen?.(p)} className="w-full text-left card p-4 flex items-center justify-between">
            <div>
      ...
      <Button className="fixed right-4 bottom-[calc(var(--tabbar-h)+20px)] rounded-full shadow-lg" onClick={onCreate}>+ Submit Payment</Button>
    </div>
  );
}
