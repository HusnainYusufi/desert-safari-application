        ...
        {drivers.map(d => (
          <div key={d.id} className="card p-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button variant="outline" onClick={()=>onEdit?.(d)}>Edit</Button>
            </div>
          </div>
        ))}
      </div>
      <Button className="fixed right-4 bottom-[calc(var(--tabbar-h)+20px)] rounded-full shadow-lg" onClick={onAdd}>+ Add Driver</Button>
    </div>
  );
}
